# *********************************************************************
# FUNCTION TO INTEGRATE TRANSFORMED Y-SOLUTIONS THROUGH A SOLID LAYER (n>=1)
#
...
from LOADGF.LN import f_solid_Z
from LOADGF.LN import Z2Y

def main(Zi,int_start,int_stop,num_soln,backend,nstps,abs_tol,rel_tol,\
	n,tck_lnd,tck_mnd,tck_rnd,tck_gnd,wnd,ond,piG,m,alpha):
    ...
    # Initialize Solver
    solver = ode(f_solid_Z.main).set_integrator(backend,atol=abs_tol,rtol=rel_tol,nsteps=nstps)
    solver.set_initial_value(Zi,int_start).set_f_params(n,tck_lnd,tck_mnd,tck_rnd,tck_gnd,wnd,ond,piG,m,alpha)
    ...
    for oo in range(0,len(sint)):
        Yint[oo] = Z2Y.main(sint[oo],alpha,Zint[oo])

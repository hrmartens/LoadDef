...
def main(si,n,tck_lnd,tck_mnd,tck_rnd,wnd,piG):
    ...
    Y31 = -(si**(n+1.)/(2.*n+3.))*(0.5*h1*psi1_n - phi1_np1)
    Y41 = mndi*(si**n)*(phi1_n - (1./(2.*n+3.))* \
	((n-1.)*h1*psi1_n + 2.*(f1+1.)*phi1_np1))
    ...
    Y32 = -(si**(n+1.)/(2.*n+3.))*(0.5*h2*psi2_n - phi2_np1)
    Y42 = mndi*(si**n)*(phi2_n - (1./(2.*n+3.))* \
        ((n-1.)*h2*psi2_n + 2.*(f2+1.)*phi2_np1))
    ...
    # THIRD SOLUTION
    Y13 = n*(si**(n-1.))
    Y23 = 2.*mndi*n*(n-1.)*(si**(n-2.))
    Y33 = si**(n-1.)
    Y43 = 2.*mndi*(n-1.)*(si**(n-2.))
    Y53 = (n*gamma-(wnd**2))*(si**n)
    Y63 = (2.*n+1.)*((n*gamma-(wnd**2))*(si**(n-1.))) - 3.*n*gamma*(si**(n-1.))
    ...
    Y = np.array([[Y11, Y21, Y31, Y41, Y51, Y61], \

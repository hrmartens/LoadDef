...
from LOADGF.LN import f_solid_noGrav
from LOADGF.LN import f_solid_linear_noGrav
...
def main(Yi,int_start,int_stop,num_soln,backend,nstps,\
    ...
    # Initialize Solver
    if (kx == 1): 
        solver = ode(f_solid_linear_noGrav.main)
        ...
        solver = ode(f_solid_noGrav.main).set_integrator(backend,atol=abs_tol,rtol=rel_tol,nsteps=nstps)

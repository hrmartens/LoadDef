...
import math
import sys
from LOADGF.LN import integrate_fullEarth_n0_noGrav
from LOADGF.LN import integrate_fullEarth_noGrav
from LOADGF.LN import integrate_mantle_noGrav
from LOADGF.LN import apply_boundary_conditions_n0_noGrav
from LOADGF.LN import apply_boundary_conditions_noGrav
from LOADGF.LN import evaluate_load_ln_n0_noGrav
from LOADGF.LN import evaluate_load_ln_noGrav
from LOADGF.LN import evaluate_potential_ln_n0
from LOADGF.LN import evaluate_potential_ln_noGrav
from LOADGF.LN import evaluate_stress_ln_n0_noGrav
from LOADGF.LN import evaluate_stress_ln_noGrav
from LOADGF.LN import evaluate_shear_ln_n0
from LOADGF.LN import evaluate_shear_ln_noGrav
from LOADGF.LN import compute_solutions_n0_noGrav
from LOADGF.LN import compute_solutions_noGrav
...
def main(n,s_min,tck_lnd,tck_mnd,tck_rnd,tck_gnd,wnd,ond,piG,sic,soc,small,\
    ...
    if (n == 0):
        ...
        Y1, sint_mt = integrate_fullEarth_n0_noGrav.main(n,s_min,tck_lnd,tck_mnd,tck_rnd,tck_gnd,wnd,ond,piG,sic,soc,small,\
        ...
        Y = fundamental_solutions_homsph_n0_noGrav.main(1.,n,tck_lnd,tck_mnd,tck_rnd,wnd,piG)
        Y1i = Y[0,:]
        ...
        m_load,m_pot,m_str,m_shr = apply_boundary_conditions_n0_noGrav.main(n,Y1[-1],gnd[-1],piG)

        # Compute Y Solutions
        Y_load = compute_solutions_n0_noGrav.main(Y1,m_load)
        Y_pot  = compute_solutions_n0_noGrav.main(Y1,m_pot) 
        Y_str  = compute_solutions_n0_noGrav.main(Y1,m_str)
        Y_shr  = compute_solutions_n0_noGrav.main(Y1,m_shr) 
        ...
        for kk in range(0,numrad):
            ...
            hprime[kk],nlprime[kk],nkprime[kk] = evaluate_load_ln_n0_noGrav.main(n,Y_load[csidx,:],adim,gsdim,T_sc,L_sc)
	    # Compute Potential Love Numbers
            hpot[kk],nlpot[kk],nkpot[kk] = evaluate_potential_ln_n0.main(n) # gravity makes no difference anyway
            # Compute Stress Love Numbers
            hstr[kk],nlstr[kk],nkstr[kk] = evaluate_stress_ln_n0_noGrav.main(n,Y_str[csidx,:],adim,gsdim,T_sc,L_sc)
            # Compute Shear Love Numbers: A Purely Shear Force Cannot Generate a Degree-0 Harmonic Response
            hshr[kk],nlshr[kk],nkshr[kk] = evaluate_shear_ln_n0.main(n) # gravity makes no difference anyway
    ...
    # Mars model from Khan et al. --> nmaxfull = 6
        ...
        # Perform the Integration Through Full Earth
        Y1, Y2, sint_mt = integrate_fullEarth_noGrav.main(n,s_min,tck_lnd,tck_mnd,tck_rnd,tck_gnd,wnd,ond,piG,sic,soc,small,\
            num_soln,backend,abs_tol,rel_tol,nstps,order,adim,gsdim,L_sc,T_sc,inf_tol,s,kx=kx)

        # Compute Solutions For Homogeneous Sphere (Size of Earth, for Different n)
        Y1_shape = np.asarray(Y1).shape
        Y = fundamental_solutions_homsph_noGrav.main(1.,n,tck_lnd,tck_mnd,tck_rnd,wnd,piG)
        Y1i = Y[0,:]
        Y2i = Y[1,:]
        Y1 = np.ones((Y1_shape[0],Y1_shape[1]))*Y1i
        ...
        # Apply Boundary Conditions at Surface
        m_load,m_pot,m_str,m_shr = apply_boundary_conditions_noGrav.main(n,Y1[-1],Y2[-1],gnd[-1],piG)

        # Compute Y Solutions
        Y_load = compute_solutions_noGrav.main(Y1,Y2,m_load)
        Y_pot  = compute_solutions_noGrav.main(Y1,Y2,m_pot)
        Y_str  = compute_solutions_noGrav.main(Y1,Y2,m_str)
        Y_shr  = compute_solutions_noGrav.main(Y1,Y2,m_shr)
        ...
        for kk in range(0,numrad):
            if (numrad == 1):
                ...
                crad = eval_radii[kk]
            # Compute Load Love Numbers
            hprime[kk],nlprime[kk],nkprime[kk] = evaluate_load_ln_noGrav.main(n,Y_load[csidx,:],adim,gsdim,T_sc,L_sc)
            # Compute Potential Love Numbers
            hpot[kk],nlpot[kk],nkpot[kk] = evaluate_potential_ln_noGrav.main(n,Y_pot[csidx,:],adim,gsdim,T_sc,L_sc)
            # Compute Stress Love Numbers
            hstr[kk],nlstr[kk],nkstr[kk] = evaluate_stress_ln_noGrav.main(n,Y_str[csidx,:],adim,gsdim,T_sc,L_sc)
        ...
        Y1, Y2, sint_mt = integrate_mantle_noGrav.main(n,tck_lnd,tck_mnd,tck_rnd,tck_gnd,wnd,ond,piG,\

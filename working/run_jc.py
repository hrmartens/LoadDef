...
if (rank == 0):
    jacmat = np.zeros((numel*3, total_cells)) # Multiplication by 3 for 3 spatial dimensions (e,n,u)
    jacrows = np.empty((numel*3,),dtype='U10') # Assumes that station names are no more than 9 characters in length (with E, N, or U also appended)
...
for jj in range(0,numel):
    ...
    if (rank == 0):
        ...
        jacmat[idxn,:] = ndisp
        jacmat[idxu,:] = udisp
        jacrows[idxe] = (sname + 'E')
        jacrows[idxn] = (sname + 'N')
        jacrows[idxu] = (sname + 'U')
        sclat[idxe] = slat
        sclat[idxn] = slat
        sclat[idxu] = slat
        sclon[idxe] = slon
...
if (rank == 0):
    ...
    f_out = ("jacobian_" + rfm + "_" + outstr + ".nc")
    ...
    jacmat_shape = jacmat.shape
    num_rows = jacmat_shape[0]
    num_cols = jacmat_shape[1]
    ...
    jacobian = dataset.createVariable('jacobian',float,('nstacomp','nloadcell'))
    ...
    jacobian.units = 'mm'
    jacobian.long_name = 'displacement_mm'
    ...
    #  sta_comp_id[:] = netCDF4.stringtochar(np.array(jacrows,dtype='S10'))
    #  load_cell_id[:] = netCDF4.stringtochar(np.array(load_cells,dtype='S10'))
    sta_comp_id._Encoding = 'ascii'
    sta_comp_id[:] = np.array(jacrows,dtype='S10')

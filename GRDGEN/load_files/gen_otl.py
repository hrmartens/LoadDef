...
# *********************************************************************
# PROGRAM TO GENERATE GRIDS FOR OCEANIC TIDAL LOADING
...
# 6. Write Load Information to a GMT-formatted File? (Lon, Lat, Amplitude)
write_gmt = False
...
if (ftype == 1):
    ...
    llat,llon,amp,pha,llat1dseq,llon1dseq,amp2darr,pha2darr = read_tpxo8atlas.main(loadfile)
    model = ("TPXO8-Atlas")
...
if (write_gmt == True):
    ...
    otl_file = ("../../output/Grid_Files/GMT/OTL/height-anomaly_" + otl_out)
    otl_file_pressure = ("../../output/Grid_Files/GMT/OTL/pressure_" + otl_out)
    # Prepare Data
    all_otl_data = np.column_stack((llon,llat,amp))
    all_otl_data_pressure = np.column_stack((llon,llat,amp*9.81*1027.5))
    # Write Data to File
    np.savetxt(otl_file, all_otl_data, fmt='%f %f %f')
    np.savetxt(otl_file_pressure, all_otl_data_pressure, fmt='%f %f %f')

# Output OTL Grid to File for Use with LoadDef
if (write_nc == True):
